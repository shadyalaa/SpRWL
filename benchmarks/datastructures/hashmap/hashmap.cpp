...
#define DEFAULT_LOOKUP_MULTIPLIER		1
...
unsigned int lookup_multiplier;
...
long set_contains(TM_ARGDECL long  val_array[])
{
    long res = 0;
    int mode = 0;
    ro = 1;
    TM_BEGIN_EXT(2, ro);
		for(int lookup_m = 0; lookup_m < lookup_multiplier; lookup_m++){    		
			res = mode == 0 ? priv_lookup_htm(TM_ARG val_array[lookup_m]) : priv_lookup_stm(TM_ARG val_array[lookup_m]);
		}
    TM_END();
...
void *test(void *data)
{
  ...
  while (myOps > 0) {
    ...
    if (op < update) {
	  ...
	  long tmp[lookup_multiplier];
	  for(int i = 0; i < lookup_multiplier; i++)
      	tmp[i] = (rand_r(&mySeed) % range) + 1;
...
MAIN(argc, argv) {
  ...
  lookup_multiplier = DEFAULT_LOOKUP_MULTIPLIER;
  N_BUCKETS = 512;

  while(1) {
    i = 0;
    c = getopt_long(argc, argv, "hd:i:n:b:r:s:u:m:a:", long_options, &i);
    ...
    switch(c) {
     ...
     case 'h':
       printf("intset -- STM stress test "
              ...
              "  -m, --multiplier <int>\n"
              "        # of reads compared to update transactions writes (default=" XSTR(DEFAULT_UPDATE) ")\n"
         );
       exit(0);
     case 'd':
