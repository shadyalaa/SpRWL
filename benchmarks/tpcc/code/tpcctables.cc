...
void TPCCTables::payment(TM_ARGDECL int64_t warehouse_id, int64_t district_id, int64_t c_warehouse_id,
	...
	/*if(local_exec_mode == 1 || local_exec_mode == 3){
		SLOW_PATH_SHARED_WRITE_D(w->w_ytd, SLOW_PATH_SHARED_READ_D(w->w_ytd)+h_amount);
		District* d = findDistrict(TM_ARG warehouse_id, district_id);
		SLOW_PATH_SHARED_WRITE_D(d->d_ytd, SLOW_PATH_SHARED_READ_D(d->d_ytd)+h_amount);
	} else{*/
		FAST_PATH_SHARED_WRITE_D(w->w_ytd, FAST_PATH_SHARED_READ_D(w->w_ytd)+h_amount);
		FAST_PATH_SHARED_WRITE_D(d->d_ytd, FAST_PATH_SHARED_READ_D(d->d_ytd)+h_amount);
	//}
	...
	output->c_data[0] = '\0';*/
		...
		FAST_PATH_SHARED_WRITE_D(customer->c_balance, FAST_PATH_SHARED_READ_D(customer->c_balance)-h_amount);
		FAST_PATH_SHARED_WRITE_D(customer->c_ytd_payment, FAST_PATH_SHARED_READ_D(customer->c_ytd_payment)+h_amount);
		FAST_PATH_SHARED_WRITE(customer->c_payment_cnt,  FAST_PATH_SHARED_READ(customer->c_payment_cnt)+1);
