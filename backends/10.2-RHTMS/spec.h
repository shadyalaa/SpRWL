...
#define TM_INIT_MUT(lock){\
        ...
        snzi_init(&(lock->readers_active));\
	lock->clocks_w = (padded_scalar_ll*)malloc(NR_CPUS*sizeof(padded_scalar_ll));\
	lock->clocks_r = (padded_scalar_ll*)malloc(NR_CPUS*sizeof(padded_scalar_ll));\
	lock->avg_cycles = (padded_scalar_ll*)malloc(NR_TYPES*sizeof(padded_scalar_ll));\
        lock->is_waiting = (padded_scalar_ll*)malloc(NR_CPUS*sizeof(padded_scalar_ll));\
	for(int i = 0; i < NR_CPUS; i++){\
		lock->clocks_w[i].counter = 0;\
		lock->clocks_r[i].counter = 0;\
		lock->is_waiting[i].counter = 0;\
        }\
	for(int i = 0; i < NR_TYPES; i++){\
		lock->avg_cycles[i].counter = 0;\
	}\
}
...
#define WRITER_WAIT_FUNCTION(lock){\
	...
	if(wait){\
		wait -= lock->avg_cycles[t_type].counter/1;\
...
#define ACQUIRE_WRITE_LOCK(lock) {\
	...
	while (1){\
		...
		if (tries <= 0) {\
			USE_GL(lock);\
			int index = 0;\
			while(snzi_query(&(lock->readers_active))) cpu_relax();\
			START_RDTSC_W2(lock); \
			break;\
		}\
	}\
...
#define READ_MODE(lock){\
	...
	while(1){\
		snzi_inc(&(lock->readers_active), local_thread_id);\
		__sync_synchronize();\
		if(IS_LOCKED((&(lock->writer_lock)))){\
			snzi_dec(&(lock->readers_active), local_thread_id);\
			while(IS_LOCKED((&(lock->writer_lock)))){\
				cpu_relax();\
			}\
			continue;\
...
#define RELEASE_WRITE_LOCK(lock){\
	if (!local_exec_mode) {\
		if(snzi_query(&(lock->readers_active))) ABORT_HTM(90);\
		END_HTM();\
